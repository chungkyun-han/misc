...
class MasterProblem:
    def __init__(self, maxValue, itemLengths, itemDemands, initialPatterns, problemname):
        ...
        for i, x in enumerate(
                itemDemands):  # create variables & set the constraints, in other words: set the minimum amount of items to be produced
        ...
        for i, x in enumerate(self.initialPatterns):  # save initial patterns and set column constraints
            ...
            var = LpVariable("Pat" + str(i), 0, None, LpContinuous, lpSum(self.obj + [self.constraintList[v] for v in
                                                                                      temp]))  # create decision variable: will determine how often pattern x should be produced
    ...
    def addPattern(self, pattern):  # add new pattern to existing model
        ...
        var = LpVariable("Pat" + str(len(self.initialPatterns)), 0, None, LpContinuous,
                         lpSum(self.obj + [pattern[v] * self.constraintList[v] for v in temp]))
        self.PatternVars.append(var)
...
class SlaveProblem:
    def __init__(self, duals, itemLengths, maxValue):
        ...
        self.slaveprob += -lpSum(
            [duals[i] * x for i, x in enumerate(self.varList)])  # use duals to set objective coefficients
